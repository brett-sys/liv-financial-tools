"""Calendar integration for Call Logger.

- Brett: Google Calendar URL (opens directly in gcal)
- Kevin: .ics file download (opens in Outlook/any calendar app, no login needed)
"""
...
def generate_ics(contact_name, phone_number, follow_up_date, notes=""):
    """Generate an .ics file string for the follow-up event."""
...
def get_calendar_urls(agent_name, contact_name, phone_number,
    ...
    """Return dict with both gcal and ics URLs so the user can pick."""
    title, description, start_dt, end_dt = _build_event_data(
        contact_name, phone_number, follow_up_date, notes
    )
