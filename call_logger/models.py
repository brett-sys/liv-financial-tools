"""SQLite database models and query helpers for Call Logger."""
...
def init_db():
    ...
    with get_db() as conn:
        ...
        """)
        # Migration: add agent_name to existing tables that lack it
        try:
...
def log_call(agent_name, contact_name, phone_number, call_datetime, direction,
    ...
    with get_db() as conn:
        cur = conn.execute(
            """INSERT INTO calls
               (agent_name, contact_name, phone_number, call_datetime, direction, outcome, notes, follow_up_date)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            (agent_name, contact_name, phone_number, call_datetime, direction,
             outcome, notes, follow_up_date or None),
        )
        conn.commit()
...
def update_call(call_id, **fields):
    """Update an existing call record with the given fields."""
    allowed = {
...
def get_calls(limit=200, offset=0, direction=None, outcome=None,
    ...
    """Get calls with optional filters, most recent first."""
    clauses = []
    params = []
...
def _week_bounds():
    """Return (monday_str, friday_str) for the current work week."""
    today = datetime.now()
...
def get_week_calls():
    """Get all calls for the current Mon-Fri work week."""
...
def get_contact_suggestions(query, limit=8):
    """Return distinct contact names/numbers matching a partial query."""
    with get_db() as conn:
        rows = conn.execute(
